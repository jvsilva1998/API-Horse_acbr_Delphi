...
uses
 ...
 function emitirRECIBO(usuario:TJSonValue; dados:dadosRecibo): TJSonObject;
 function consultarRECIBO(usuario:TJSonValue; parametros:dadosConsultaRecibo; pagina:Integer;registros:Integer): TJSonObject;
 ...
 begin
    ...
    try
      {web service}
       try
         ...
         nfe_componente.Configuracoes.Arquivos.PathSchemas                := './Schemas';
          ...
          if usuario.FindValue('nfceambiente').Value = '1' then
            begin
           ...
           nfe_componente.NotasFiscais[0].nfe.emit.enderemit.XPAIS    := 'BRASIL';
            ...
            on E: Exception do
             begin
              ...
              {dados do cliente}
               try
                     nfe_componente.NotasFiscais[0].nfe.Dest.indIEDest          := inNaoContribuinte;
                     ...
                     if funcoes.somenteNumeros(dados.destinatario.infoEndereco.numero) <> '' then
                       ...
                       if Trim(dados.destinatario.infoEndereco.codigoCidade) <> '' then
                          begin
                 ...
                 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                   ...
                   with nfe_componente.NotasFiscais[0].nfe do
                    begin
                       ...
                       total.ICMSTot.vNF      := total.ICMSTot.vProd - total.ICMSTot.vDesc + total.ICMSTot.vIPI + Total.ICMSTot.vOutro + Total.ICMSTot.vST +Total.ICMSTot.vFrete +Total.ICMSTot.vSeg + Total.ICMSTot.vII+total.ICMSTot.vST;
      ...
      with nfe_componente.NotasFiscais[0].nfe do
       begin
            ide.demi := StrToDateTime(formatdatetime('dd/mm/yyyy',now)+' '+formatdatetime('hh:mm:ss',now - StrToTime('00:00') ));
          ...
          //  if fecha.hr_verao = '2' then ide.demi := StrToDateTime(formatdatetime('dd/mm/yyyy',now)+' '+formatdatetime('hh:mm:ss',now + StrToTime('01:00') ));
            ...
            ide.modelo                                        := 65;
            Ide.tpImp                                         := tiNFCe;
            nfe_componente.Configuracoes.Geral.ModeloDF       := moNFCe;
           ...
           // ide.demi    := StrToDateTime(formatdatetime('dd/mm/yyyy',now));
            ...
            if dados.frete.modoFrete.tipoFrete <> 4 then
                       begin
                         nfe_componente.NotasFiscais[0].nfe.ide.indPres  := pcNao;
       ...
       {certificado digital}
        try
          ...
          nfe_componente.Configuracoes.Geral.VersaoQRCode            := veqr200;
          nfe_componente.Configuracoes.Geral.IdCSC                   := usuario.FindValue('nfceidcsc').Value;
          nfe_componente.Configuracoes.Geral.CSC                     := usuario.FindValue('nfcecsc').Value;
       ...
       try
        ...
        try
          ...
          query.SQL.Add(',''' +usuario.FindValue('nfceambiente').Value+ '''');
          query.SQL.Add(',''' +'2'+ ''''); //CUPOM FISCAL
     ...
     if parametros.numeroNota <> ''  then
          begin
             ...
             sql          := 'SELECT '+
                          ...
                          ' AND tiponota = 2 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio DESC LIMIT '+IntToStr(pagina-1)+','+IntToStr(registros)+'  ';
          end;
     ...
     if parametros.numeroDocumento <> ''  then
          begin
                       ...
                       sql :=  'SELECT '+
                          ...
                          ' AND tiponota = 2 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio ASC LIMIT '+IntToStr(pagina-1)+','+IntToStr(registros)+'  ';
          end;
      ...
      if (parametros.chaveAcesso = '') and (parametros.numeroNota = '') and (parametros.numeroDocumento = '') then
       begin
        sql               := 'SELECT '+
                          ...
                          ' AND tiponota = 2 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio ASC LIMIT '+IntToStr(registros)+' OFFSET '+IntToStr((pagina-1) * registros)+' ';
    ...
    try
      ...
      with nfe_componente.NotasFiscais[0].nfe do
       begin
        ...
        {salva recibo de venda}
        try


          tituloMensagem   := 'Recibo gerado com sucesso';
          codigoHttp       :=  201;
          ...
          query.SQL.Clear;
          query.Close;
          query.SQL.Add('SELECT COUNT(*)+1 AS TOTAL  FROM notas_nfe WHERE tiponota =3 AND  codusuario = '''+usuario.FindValue('codigousuario').Value+'''');
          query.Open();

          numeroRecibo := query.FieldByName('TOTAL').AsString;
          nfe_componente.NotasFiscais[0].nfe.InfAdic.infCpl  := numeroRecibo;

          nfe_componente.NotasFiscais.GravarXML('./novo.xml');
          ...
          query.SQL.Add(',''' +'1'+ '''');
          query.SQL.Add(',''' +'1'+ '''');
          query.SQL.Add(',''' +numeroRecibo+ '''');
          query.SQL.Add(',''' +codigoRecibo+ '''');
          query.SQL.Add(',:xml');
          query.SQL.Add(',''' +'1'+ '''');
          query.SQL.Add(',:valor');
          query.SQL.Add(',''' +'GERADO'+ '''');
          query.SQL.Add(',''' +FormatDateTime('YYYY-MM-DD',NOW)  + '''');
          query.SQL.Add(',''' +TimeToStr(now)+ '''');
          query.SQL.Add(',''' +dados.destinatario.info.nomeRazao+ '''');
          query.SQL.Add(',''' +dados.destinatario.info.cpfCnpj+ '''');
          query.SQL.Add(',''' +'1'+ '''');
          query.SQL.Add(',''' +'3'+ ''''); //RECIBO DE VENDA
          query.SQL.Add(',''' +usuario.FindValue('codigousuario').Value+ ''')');

          query.Params[0].DataType := ftBlob;
          query.Params[0].Value    := nfe_componente.NotasFiscais.Items[0].XML;
          ...
          query.ExecSQL;
                   ...
                   json := '{ '+
             '  "mensagemRetorno": {  '+
             '   "titulo": "'+ 'Recibo Nº '+numeroRecibo+' gerado com sucesso' +'", '+
             '   "mensagem": "'+ codigoRecibo  +'" '+
             ' }, '+
             ' "dadosRetorno": [], '+
             ' "codigoHttp": "'+ IntToStr(codigoHttp) +'" '+
             ' }' ;
                ...
                begin
                 ...
                 tituloMensagem := 'Erro ao gravar Recibo ';
                 codigoHttp     :=  500;
                 mensagem       := funcoes.RemoveAcento(e.Message);
 ...
 function consultarRECIBO(usuario:TJSonValue; parametros:dadosConsultaRecibo;pagina:Integer;registros:Integer): TJSonObject;
  ...
  begin
     ...
     query.Connection := conexao;
        ...
        if parametros.nomeCliente <> ''  then
          begin
                       sql :=  'SELECT '+
                          'id,'+
                           // 'serienota,'+
                         ...
                         // 'statusnota,'+
                          ...
                          'nomedestinatario,'+
                          'documentodestinatario '+
                          'FROM notas_nfe where nomedestinatario like ''%'+parametros.nomeCliente+'%'' AND dataenvio BETWEEN  '''+FormatDateTime('YYYY-MM-DD',parametros.dataInicio)+''' AND '''+FormatDateTime('YYYY-MM-DD',parametros.dataFim)+''''+
                          ' AND tiponota = 3 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio ASC LIMIT '+IntToStr(pagina-1)+','+IntToStr(registros)+'  ';
          end;



     if parametros.numeroRecibo <> ''  then
          begin
                       sql :=  'SELECT '+
                          'id,'+
                         // 'serienota,'+
                         ...
                         // 'statusnota,'+
                          ...
                          'documentodestinatario '+
                        //  'codusuario '+
                          'FROM notas_nfe  where protocolo = '''+parametros.numeroRecibo+''' AND tiponota = 3 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio ASC LIMIT '+IntToStr(pagina-1)+','+IntToStr(registros)+'  ';
          end;


         if parametros.documentoCliente <> ''  then
          begin
                       sql :=  'SELECT '+
                          'id,'+
                           // 'serienota,'+
                         ...
                         // 'statusnota,'+
                          ...
                          'nomedestinatario,'+
                          'documentodestinatario '+
                          'FROM notas_nfe  where documentodestinatario = '''+parametros.documentoCliente+''' AND dataenvio BETWEEN  '''+FormatDateTime('YYYY-MM-DD',parametros.dataInicio)+''' AND '''+FormatDateTime('YYYY-MM-DD',parametros.dataFim)+''''+
                          ' AND tiponota = 3 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio ASC LIMIT '+IntToStr(pagina-1)+','+IntToStr(registros)+'  ';
          end;



      if (parametros.numeroRecibo = '') and (parametros.documentoCliente = '' )  and (parametros.nomeCliente = '') then
       begin
                       sql :=  'SELECT '+
                          'id,'+
                          // 'serienota,'+
                         ...
                         // 'statusnota,'+
                          ...
                          'nomedestinatario,'+
                          'documentodestinatario '+
                          'FROM notas_nfe  where dataenvio BETWEEN  '''+FormatDateTime('YYYY-MM-DD',parametros.dataInicio)+''' AND '''+FormatDateTime('YYYY-MM-DD',parametros.dataFim)+''''+
                          ' AND tiponota = 3 AND codusuario='''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataenvio ASC LIMIT '+IntToStr(pagina-1)+','+IntToStr(registros)+'  ';
       end;
              ...
              begin
               ...
               mensagem        := 'Nenhuma nfe encontrada';
                   ...
                   begin
                          codigoHttp := 200;
                          LJSONArray            := query.ToJSONArray;
                          dados := '"recibos":'+ LJSONArray.Format;
                          LJSONArray.Free;
                   end;
           except
            on E: Exception do
  ...
  begin
    ...
    query.Connection     := conexao;
     ...
     try
      conexao.Open('DriverID='+funcoes.dbDriver+';Database='+funcoes.dbNome+';User_Name='+funcoes.dbUser+';Password='+funcoes.dbSenha+';Port='+funcoes.dbPorta+';Server='+funcoes.dbHost);
      query.close;
      query.SQL.Add('SELECT statusnota,ambiente,xmlnota FROM notas_nfe WHERE id = '''+id+''' and tiponota =2 AND codusuario = '''+usuario.FindValue('codigousuario').Value+'''');
      query.open;
                                                 ...
                                                 if nfe_componente.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.cStat = 135 then
                                                     begin
                                                         ...
                                                         try
                                                              ...
                                                              query.SQL.Clear;
                                                              query.SQL.Add('UPDATE notas_nfe SET  statusnota = '''+'CANCELADA'+'''  where tiponota=2 and  id = '''+id+''' AND  codusuario = '''+usuario.FindValue('codigousuario').Value+'''');
                                                              query.ExecSQL;
  ...
  begin
     ...
     query.Connection := conexao;
        try
           conexao.Open('DriverID='+funcoes.dbDriver+';Database='+funcoes.dbNome+';User_Name='+funcoes.dbUser+';Password='+funcoes.dbSenha+';Port='+funcoes.dbPorta+';Server='+funcoes.dbHost);
           query.Close;
           query.SQL.Add('select idEvento,idNfe,tipoEvento,dataEvento,TIME_FORMAT(horaEvento, "%H:%i:%s") AS horaEvento,protocolo from eventosnfe where tpnpta =2 and idNfe = '''+id+''' and codigoUsuario = '''+usuario.FindValue('codigousuario').Value+''' ORDER BY dataEvento ASC LIMIT '+IntToStr(registros)+' OFFSET '+IntToStr((pagina-1) * registros)+' ');
           query.Open;
                 ...
                 try
                  ...
                  {inutilizar NF-E}
                   try
                     nfe_componente.WebServices.Inutiliza(usuario.FindValue('cnpjempresa').Value,funcoes.RemoveAcento(justificativa),StrToInt(FormatDateTime('yy',now)),55,serie,numeroInicio,numeroFim);
                        ...
                        on E: Exception do
                         begin
                            tituloMensagem := 'Erro ao inutilizar NFe';
                            mensagem       :=  funcoes.RemoveAcento(e.Message);
                            codigoHttp     :=  200;
                            funcoes.logErro(e.Message);
                  ...
                  if nfe_componente.WebServices.Inutilizacao.CStat <> 102 then
                   ...
                   try
                        ...
                        query.SQL.Add(',:xml');
                        query.SQL.Add(',''' +'2'+''''); // inutiliza nfce
                        query.SQL.Add(',''' +usuario.FindValue('codigousuario').Value+ ''')');
  ...
  begin
     ...
     query.Connection := conexao;
        try
           conexao.Open('DriverID='+funcoes.dbDriver+';Database='+funcoes.dbNome+';User_Name='+funcoes.dbUser+';Password='+funcoes.dbSenha+';Port='+funcoes.dbPorta+';Server='+funcoes.dbHost);
           query.Close;
           query.SQL.Add('select id,data,TIME_FORMAT(hora, "%H:%i:%s") AS hora,serie,numeroInicio,numeroFim,tpAmbiente,justificativa,protocolo from inutilizacaonfe where tpnota =2 and codigoUsuario = '''+usuario.FindValue('codigousuario').Value+''' ORDER BY data ASC LIMIT '+IntToStr(registros)+' OFFSET '+IntToStr((pagina-1) * registros)+' ');
           query.Open;
