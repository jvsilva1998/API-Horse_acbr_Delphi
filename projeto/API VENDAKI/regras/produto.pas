...
uses funcoes;
 ...
 function verificaExisteUPDATE(codigointerno:string;ean:string;codigoProduto:string;codigoCliente:string;conexao:TFDConnection):Boolean;
  ...
  begin
    ...
    if (ean = '') AND (codigoInterno <> '') then
          begin
            sql := 'SELECT count(*) as total FROM produtos WHERE cod_produto <> '''+codigoProduto+''' AND codigointerno = '''+codigointerno+''' AND  cod_usuario = ''' +codigoCliente+'''';
          end;

    if (ean <> '') AND (codigoInterno = '') then
          begin
            sql := 'SELECT count(*) as total FROM produtos WHERE cod_produto <> '''+codigoProduto+'''  AND codigobarras = '''+ean+''' AND cod_usuario = ''' +codigoCliente+'''';
          end;


    if (ean <> '') AND (codigoInterno <> '') then
          begin
            sql := 'SELECT count(*) as total FROM produtos WHERE cod_produto <> '''+codigoProduto+'''  AND codigointerno = '''+codigointerno+''' OR codigobarras = '''+ean+'''  AND  cod_produto <> '''+codigoProduto+''' AND cod_usuario = ''' +codigoCliente+'''';
          end;
  ...
  begin
    ...
    if ean = '' then
       begin
        ...
        try
         query.Close;
         query.SQL.Add('SELECT count(*) as total FROM produtos '+interno+' '+barras+' AND cod_usuario = '''+codigoCliente+'''');
         query.Open;

         if query.FieldByName('total').AsInteger >= 1  then
            begin
              Result := False;
            end else
                  begin
     ...
     if ean <> '%'  then
        ...
        try
           ...
           query.SQL.Add(' SELECT '+
                          ...
                          'perfil_tributario.PERC_PIS,perfil_tributario.PERC_IPI, '+
