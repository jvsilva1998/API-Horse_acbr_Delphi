...
type
  ...
  Iusuarios = interface(IInvokable)
    ...
    [Route('login/recupera')][httppost] function recuperaSenha(email:string):TJSONObject;
    ...
    [httpput]  function  nfce(serie,numero,ambiente,larguraImpressao:int64;csc:string;Idcsc:string;observacoes:string):TJSONObject;
    [httpput]  function  nfe(serie,numero,ambiente:int64;observacoes:string):TJSONObject;
    [Route('vendas/cadastrar/')] [httppost] function nova_venda(dados:parametros.venda):TJSONObject;
    [Route('credito/cadastrar/')] [httppost] function novo_credito(dados:parametros.credito):TJSONObject;
    [Route('creditos/buscar/')] [httpget] function buscar_creditos(pagina,registros:integer):TJSONObject;
    [Route('creditos/saldo/')] [httpget] function saldo_creditos:TJSONObject;
    [Route('cobrancas/limite/')] [httpget] function limite_cobrancas:TJSONObject;
  ...
  private
   ...
   function  nfce(serie,numero,ambiente,larguraImpressao:int64;csc:string;Idcsc:string;observacoes:string):TJSONObject;
   function  nfe(serie,numero,ambiente:int64;observacoes:string):TJSONObject;
   function  removerLogo:TJSONObject;
   function  novo_credito(dados:parametros.credito):TJSONObject;
   function  buscar_creditos(pagina,registros:integer):TJSONObject;
   function  saldo_creditos:TJSONObject;
   function  limite_cobrancas:TJSONObject;
   function  recuperaSenha(email:string):TJSONObject;
  end;
...
function Tusuarios.limite_cobrancas: TJSONObject;
 var
   usuarios    : TJSONObject;
   resultado  : TJSONObject;
 begin
    ...
    if usuarios.FindValue('codigoHttp').value = '200' then
      begin
            resultado := usuario.limiteCobranca(usuarios.P['mensagemRetorno'].FindValue('mensagem').Value);
...
function Tusuarios.nfce(serie,numero,ambiente,larguraImpressao:int64;csc:string;Idcsc:string;observacoes:string): TJSONObject;
  var
   resultado      : TJSONObject;
   usuarios        : TJSONObject;
 begin
    ...
    if usuarios.FindValue('codigoHttp').value = '200' then
      begin
            resultado  :=  usuario.atualizaNfce(usuarios.P['mensagemRetorno'].FindValue('mensagem').Value,serie,numero,ambiente,larguraImpressao,csc,idcsc,observacoes);
            result     :=  TJSonObject.ParseJSONValue(resultado.P['mensagemRetorno'].ToString) as TJSonObject;
            funcoes.statusHTTP(strtoint(resultado.FindValue('codigoHttp').value));
      end else
           begin
...
function Tusuarios.nfe(serie, numero, ambiente: int64;observacoes:string): TJSONObject;
  var
   resultado      : TJSONObject;
   usuarios        : TJSONObject;
 begin
    ...
    if usuarios.FindValue('codigoHttp').value = '200' then
      begin
            resultado  :=  usuario.atualizaNfe (usuarios.P['mensagemRetorno'].FindValue('mensagem').Value,serie,numero,ambiente,observacoes);
            result     :=  TJSonObject.ParseJSONValue(resultado.P['mensagemRetorno'].ToString) as TJSonObject;
            funcoes.statusHTTP(strtoint(resultado.FindValue('codigoHttp').value));
      end else
           begin
...
function Tusuarios.novo_credito(dados: parametros.credito): TJSONObject;
  var
   resultado      : TJSONObject;
   usuarios        : TJSONObject;
 begin
    ...
    if usuarios.FindValue('codigoHttp').value = '200' then
      begin
            resultado  :=  usuario.faturaCredito(usuarios.P['mensagemRetorno'].FindValue('mensagem').Value,dados);
            result     :=  TJSonObject.ParseJSONValue(resultado.P['mensagemRetorno'].ToString) as TJSonObject;
            funcoes.statusHTTP(strtoint(resultado.FindValue('codigoHttp').value));
      end else
           begin
...
function Tusuarios.recuperaSenha(email: string): TJSONObject;
 var
  token     : string;
  resultado : TJSonObject;
   usuarioADM     : TJSONObject;
 begin
  ...
  if usuarioADM.FindValue('codigoHttp').Value = '200' then
     begin
         resultado  :=   usuario.recuperaSenha(email);
         result     :=  TJSonObject.ParseJSONValue(resultado.P['mensagemRetorno'].ToString) as TJSonObject;
         funcoes.statusHTTP(strtoint(resultado.FindValue('codigoHttp').value));
     end
     else
...
function Tusuarios.saldo_creditos: TJSONObject;
 var
   usuarios    : TJSONObject;
   resultado  : TJSONObject;
 begin
    ...
    if usuarios.FindValue('codigoHttp').value = '200' then
      begin
            resultado := usuario.saldoCreditos(usuarios.P['mensagemRetorno'].FindValue('mensagem').Value);
            if resultado.FindValue('codigoHttp').Value = '200' then
               begin
                result      := TJSonObject.ParseJSONValue(resultado.P['dadosRetorno'].ToString) as TJSonObject;
               end else
...
function Tusuarios.buscar_creditos(pagina, registros: integer): TJSONObject;
 var
   login     : TJSONObject;
   resultado : TJSONObject;
 begin
    ...
    if login.FindValue('codigoHttp').value = '200' then
      begin
            resultado := usuario.historicoCreditos(login.P['mensagemRetorno'].FindValue('mensagem').Value,pagina,registros);
            if resultado.FindValue('codigoHttp').Value = '200' then
               begin
                result      := TJSonObject.ParseJSONValue(resultado.P['dadosRetorno'].ToString) as TJSonObject;
               end else
