...
uses
 ...
 function buscaVersao:TJSONObject;
            ...
            try
                     // 1ª NOTAS FISCAIS //////////////////////////////////////////////////////////////////////////////////
                       ...
                       query.SQL.Add('SELECT SUM(valornota)   AS TOTAL from notas_nfe   WHERE statusnota = '''+'ENVIADA'+'''   AND dataenvio > (curdate() - interval 30 day)  AND codusuario = '''+cod_usuario+''' union all '+
                                     ...
                                     'SELECT COUNT(*)         AS TOTAL from eventosnfe  WHERE tipoEvento = 2 AND dataEvento > (curdate() - interval 30 day)                   AND codigoUsuario = '''+cod_usuario+''' union all '+
  ...
  begin
     ...
     query.Connection := conexao;
          ...
          try
             conexao.Open('DriverID='+funcoes.dbDriver+';Database='+funcoes.dbNome+';User_Name='+funcoes.dbUser+';Password='+funcoes.dbSenha+';Port='+funcoes.dbPorta+';Server='+funcoes.dbHost);
             query.Close;
             query.SQL.Add('select *from upcell');
             query.Open;
               ...
               if  query.RowsAffected = 0  then
                  begin
                     ...
                     mensagem        := 'Nenhum upcell encontrado';
                  end else
                       begin
                          codigoHttp := 200;
                          LJSONArray            := query.ToJSONArray;
                          dados := '"upcell":'+ LJSONArray.Format;
                          LJSONArray.Free;
                       end;
 ...
 function buscaVersao:TJSONObject;
  ...
  begin
          ...
          try
              ...
              tituloMensagem := 'Versão do kinotas';
              mensagem       := ArqIni.ReadString('configuracoes', 'versao', '');
              codigoHttp     := 200;
