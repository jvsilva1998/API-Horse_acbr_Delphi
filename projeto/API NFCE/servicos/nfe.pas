...
type
  ...
  Infe = interface(IInvokable)
    ...
    [Route('recibo/novo/')][httppost] function novoRecibo(dadods:parametros.dadosRecibo):TJSonObject;
    [Route('recibo/buscar/')][httppost] function buscarRecibo([FromPath]registros:Integer; [FromPath]pagina:integer;dadods:parametros.dadosConsultaRecibo):TJSonObject;
    ...
    [ServiceImplementation]
      ...
      private
       ...
       function  novoRecibo(dadods:parametros.dadosRecibo):TJSonObject;
       function  buscarRecibo([FromPath]registros:Integer; [FromPath]pagina:integer;dadods:parametros.dadosConsultaRecibo):TJSonObject;
  end;
...
function TInfe.alterarRascunho(codigo, jsonNota, cliente: string;valor: double): TJSonObject;
  var
...
function TInfe.buscar(registros, pagina: integer;dadods: parametros.dadosConsulta): TJSonObject;
 var
   usuario    : TJSONObject;
   resultado  : TJSONObject;
 begin
...
function TInfe.novoRecibo(dadods: parametros.dadosRecibo): TJSonObject;
  var
   resultado      : TJSONObject;
   usuario        : TJSONObject;
   tokenLogado    : string;
         ...
         usuario  := funcoes.logaToken(TXDataOperationContext.Current.Request.Headers.Get('Authorization'));
              if usuario.FindValue('codigoHttp').value = '200' then
                begin
                      ...
                      resultado  :=  regras_nfe.emitirRECIBO(usuario.P['dadosRetorno'].P['usuario'].a[0],dadods);
                      result     :=  TJSonObject.ParseJSONValue(resultado.P['mensagemRetorno'].ToString) as TJSonObject;
                      funcoes.statusHTTP(strtoint(resultado.FindValue('codigoHttp').value));
                end else
                     begin
...
function TInfe.buscarRecibo(registros, pagina: integer;
  dadods: parametros.dadosConsultaRecibo): TJSonObject;
 var
   usuario    : TJSONObject;
   resultado  : TJSONObject;
 begin
    ...
    if usuario.FindValue('codigoHttp').value = '200' then
      begin
            resultado := regras_nfe.consultarRECIBO(usuario.P['dadosRetorno'].P['usuario'].a[0],dadods,pagina,registros);
            if resultado.FindValue('codigoHttp').Value = '200' then
               begin
                result      := TJSonObject.ParseJSONValue(resultado.P['dadosRetorno'].ToString) as TJSonObject;
               end else
