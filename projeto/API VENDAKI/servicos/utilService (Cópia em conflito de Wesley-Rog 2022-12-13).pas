...
type
  ...
  Iutil= interface(IInvokable)
    ...
    [httpget]function  produto(ean:string):TJSONObject;
  end;
  ...
  private
    ...
    function  produto(ean:string):TJSONObject;
  end;
...
function TIutil.produto(ean: string): TJSONObject;
 var
   usuario     : TJSONObject;
   resultado   : TJSONObject;
   usuarioADM  : TJSONObject;
   begin
      ...
      if usuarioADM.FindValue('codigoHttp').Value = '200' then
        begin
           result      := TJSonObject.ParseJSONValue(principal.retornaNCM(ean)) as TJSonObject;
        end else
            begin
             funcoes.statusHTTP(strtoint(usuarioADM.FindValue('codigoHttp').Value));
             result  := TJSonObject.ParseJSONValue(usuarioADM.P['mensagemRetorno'].ToString) as TJSonObject;
